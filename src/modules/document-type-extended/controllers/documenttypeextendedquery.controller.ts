...
@ApiTags("DocumentTypeExtended Query")
@UseGuards(DocumentTypeExtendedAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("documenttypeextendeds/query")
export class DocumentTypeExtendedQueryController {
  #logger = new Logger(DocumentTypeExtendedQueryController.name);

  constructor(private readonly service: DocumentTypeExtendedQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all documenttypeextended with optional pagination" })
  @ApiResponse({ status: 200, type: DocumentTypeExtendedsResponse })
  @ApiQuery({ name: "options", required: false, type: DocumentTypeExtendedDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<DocumentTypeExtended>    
  ): Promise<DocumentTypeExtendedsResponse<DocumentTypeExtended>> {
    try {
     
      const documenttypeextendeds = await this.service.findAll(options);
      logger.info("Retrieving all documenttypeextended");
  ...
  @ApiOperation({ summary: "Find documenttypeextended by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter documenttypeextended', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: DocumentTypeExtendedsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "DocumentTypeExtended no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find documenttypeextendeds with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<DocumentTypeExtended>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all documenttypeextendeds" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count documenttypeextendeds with conditions" })
  @ApiResponse({ status: 200, type: DocumentTypeExtendedsResponse<DocumentTypeExtended> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades DocumentTypeExtendeds no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one documenttypeextended with conditions" })
  ...
  @ApiOperation({ summary: "Find one documenttypeextended or return error" })
  @ApiResponse({ status: 200, type: DocumentTypeExtendedResponse<DocumentTypeExtended> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad DocumentTypeExtended no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get documenttypeextended by ID" })
  @ApiResponse({ status: 200, type: DocumentTypeExtendedResponse<DocumentTypeExtended> })
  @ApiResponse({ status: 404, description: "DocumentTypeExtended not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the documenttypeextended to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<DocumentTypeExtendedResponse<DocumentTypeExtended>> {
    try {
      const documenttypeextended = await this.service.findOne({ where: { id } });
      if (!documenttypeextended) {
        throw new NotFoundException(
          "DocumentTypeExtended no encontrado para el id solicitado"
        );
      }
      return documenttypeextended;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }

...
@ApiTags("UpstreamSyncStatus Query")
@UseGuards(UpstreamSyncStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("upstreamsyncstatuss/query")
export class UpstreamSyncStatusQueryController {
  #logger = new Logger(UpstreamSyncStatusQueryController.name);

  constructor(private readonly service: UpstreamSyncStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all upstreamsyncstatus with optional pagination" })
  @ApiResponse({ status: 200, type: UpstreamSyncStatussResponse })
  @ApiQuery({ name: "options", required: false, type: UpstreamSyncStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<UpstreamSyncStatus>    
  ): Promise<UpstreamSyncStatussResponse<UpstreamSyncStatus>> {
    try {
     
      const upstreamsyncstatuss = await this.service.findAll(options);
      logger.info("Retrieving all upstreamsyncstatus");
  ...
  @ApiOperation({ summary: "Find upstreamsyncstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter upstreamsyncstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: UpstreamSyncStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "UpstreamSyncStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find upstreamsyncstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<UpstreamSyncStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all upstreamsyncstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count upstreamsyncstatuss with conditions" })
  @ApiResponse({ status: 200, type: UpstreamSyncStatussResponse<UpstreamSyncStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades UpstreamSyncStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one upstreamsyncstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one upstreamsyncstatus or return error" })
  @ApiResponse({ status: 200, type: UpstreamSyncStatusResponse<UpstreamSyncStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad UpstreamSyncStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get upstreamsyncstatus by ID" })
  @ApiResponse({ status: 200, type: UpstreamSyncStatusResponse<UpstreamSyncStatus> })
  @ApiResponse({ status: 404, description: "UpstreamSyncStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the upstreamsyncstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<UpstreamSyncStatusResponse<UpstreamSyncStatus>> {
    try {
      const upstreamsyncstatus = await this.service.findOne({ where: { id } });
      if (!upstreamsyncstatus) {
        throw new NotFoundException(
          "UpstreamSyncStatus no encontrado para el id solicitado"
        );
      }

...
import { ActiveStatusExpirableDto } from "../dtos/all-dto";
...
@ApiTags("ActiveStatusExpirable Query")
@UseGuards(ActiveStatusExpirableAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("activestatusexpirables/query")
export class ActiveStatusExpirableQueryController {
  #logger = new Logger(ActiveStatusExpirableQueryController.name);

  constructor(private readonly service: ActiveStatusExpirableQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all activestatusexpirable with optional pagination" })
  @ApiResponse({ status: 200, type: ActiveStatusExpirablesResponse })
  @ApiQuery({ name: "options", required: false, type: ActiveStatusExpirableDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ActiveStatusExpirable>    
  ): Promise<ActiveStatusExpirablesResponse<ActiveStatusExpirable>> {
    try {
     
      const activestatusexpirables = await this.service.findAll(options);
      logger.info("Retrieving all activestatusexpirable");
  ...
  @ApiOperation({ summary: "Find activestatusexpirable by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter activestatusexpirable', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ActiveStatusExpirablesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ActiveStatusExpirable no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find activestatusexpirables with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ActiveStatusExpirable>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all activestatusexpirables" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count activestatusexpirables with conditions" })
  @ApiResponse({ status: 200, type: ActiveStatusExpirablesResponse<ActiveStatusExpirable> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one activestatusexpirable with conditions" })
  ...
  @ApiOperation({ summary: "Find one activestatusexpirable or return error" })
  @ApiResponse({ status: 200, type: ActiveStatusExpirableResponse<ActiveStatusExpirable> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get activestatusexpirable by ID" })
  @ApiResponse({ status: 200, type: ActiveStatusExpirableResponse<ActiveStatusExpirable> })
  @ApiResponse({ status: 404, description: "ActiveStatusExpirable not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the activestatusexpirable to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ActiveStatusExpirableResponse<ActiveStatusExpirable>> {
    try {
      const activestatusexpirable = await this.service.findOne({ where: { id } });
      if (!activestatusexpirable) {
        throw new NotFoundException(
          "ActiveStatusExpirable no encontrado para el id solicitado"
        );
      }
      return activestatusexpirable;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }

...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class ActiveStatusExpirableRepository {
    constructor(
    ...
    async findAll(options?: FindManyOptions<ActiveStatusExpirable>): Promise<ActiveStatusExpirable[]> {
      logger.info('Ready to findAll ActiveStatusExpirable on repository:',options);
    ...
    async findById(id: string): Promise<ActiveStatusExpirable | null> {
      const tmp: FindOptionsWhere<ActiveStatusExpirable> = { id } as FindOptionsWhere<ActiveStatusExpirable>;
      logger.info('Ready to findById ActiveStatusExpirable on repository:',tmp);
    ...
    async findByField(
      ...
      page: number,
      limit: number
    ): Promise<ActiveStatusExpirable[]> {
      let options={
        where: { [field]: value },
        skip: (page - 1) * limit,
        take: limit,
      };
      logger.info('Ready to findByField ActiveStatusExpirable on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count ActiveStatusExpirable on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of ActiveStatusExpirable on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[ActiveStatusExpirable[], number]> {
      logger.info('Ready to findByField ActiveStatusExpirable on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<ActiveStatusExpirable | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding ActiveStatusExpirable.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) ActiveStatusExpirable:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<ActiveStatusExpirable> = options as FindOptionsWhere<ActiveStatusExpirable>;
      logger.info('Ready to findOneBy ActiveStatusExpirable on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<ActiveStatusExpirable> {
      logger.info('Ready to findOneOrFail ActiveStatusExpirable on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of ActiveStatusExpirable with id:  was finded on repository:', instance);

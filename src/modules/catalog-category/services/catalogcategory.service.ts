...
export class CatalogCategoryService {
  ...
  // Métodos delegados
  async findAll(options?: any): Promise<CatalogCategory[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<CatalogCategory[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<CatalogCategory[]> {
  ...
  async update(id: string, partialEntity: Partial<CatalogCategory>): Promise<CatalogCategory | null> {

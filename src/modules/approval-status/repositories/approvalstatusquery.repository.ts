...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class ApprovalStatusQueryRepository {

    //Constructor del repositorio de datos: ApprovalStatusQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination ApprovalStatus on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[ApprovalStatus[], number]> {
      logger.info('Ready to findAndCount ApprovalStatus on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of ApprovalStatus on repository:',result);
      return result;
    }

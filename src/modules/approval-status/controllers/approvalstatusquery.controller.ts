...
@ApiTags("ApprovalStatus Query")
@UseGuards(ApprovalStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("approvalstatuss/query")
export class ApprovalStatusQueryController {
  #logger = new Logger(ApprovalStatusQueryController.name);

  constructor(private readonly service: ApprovalStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all approvalstatus with optional pagination" })
  @ApiResponse({ status: 200, type: ApprovalStatussResponse })
  @ApiQuery({ name: "options", required: false, type: ApprovalStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ApprovalStatus>    
  ): Promise<ApprovalStatussResponse<ApprovalStatus>> {
    try {
     
      const approvalstatuss = await this.service.findAll(options);
      logger.info("Retrieving all approvalstatus");
  ...
  @ApiOperation({ summary: "Find approvalstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter approvalstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ApprovalStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ApprovalStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find approvalstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ApprovalStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all approvalstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count approvalstatuss with conditions" })
  @ApiResponse({ status: 200, type: ApprovalStatussResponse<ApprovalStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ApprovalStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one approvalstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one approvalstatus or return error" })
  @ApiResponse({ status: 200, type: ApprovalStatusResponse<ApprovalStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ApprovalStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get approvalstatus by ID" })
  @ApiResponse({ status: 200, type: ApprovalStatusResponse<ApprovalStatus> })
  @ApiResponse({ status: 404, description: "ApprovalStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the approvalstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ApprovalStatusResponse<ApprovalStatus>> {
    try {
      const approvalstatus = await this.service.findOne({ where: { id } });
      if (!approvalstatus) {
        throw new NotFoundException(
          "ApprovalStatus no encontrado para el id solicitado"
        );
      }

...
export class ApprovalStatusService {
  constructor(private readonly repository: ApprovalStatusRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<ApprovalStatus[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<ApprovalStatus[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<ApprovalStatus[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<ApprovalStatus | null> {
  ...
  async update(id: string, partialEntity: Partial<ApprovalStatus>): Promise<ApprovalStatus | null> {

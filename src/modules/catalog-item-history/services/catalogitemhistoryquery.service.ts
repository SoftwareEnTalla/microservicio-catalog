...
@Injectable()
export class CatalogItemHistoryQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(CatalogItemHistoryQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: CatalogItemHistoryQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${CatalogItemHistory.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const catalogitemhistorys = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de catalogitemhistorys obtenido con éxito",
        data: catalogitemhistorys,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          catalogitemhistorys.length
        ),
        count: catalogitemhistorys.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<CatalogItemHistoryResponse<CatalogItemHistory>> {
    try {
      const catalogitemhistory = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "CatalogItemHistorys no encontrados para la propiedad y valor especificado"
        );
      // Devolver catalogitemhistory
      return {
        ok: true,
        message: "CatalogItemHistorys obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<CatalogItemHistoryResponse<CatalogItemHistory>> {
    try {

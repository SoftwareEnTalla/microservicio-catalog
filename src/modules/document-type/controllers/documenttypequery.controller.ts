...
@ApiTags("DocumentType Query")
@UseGuards(DocumentTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("documenttypes/query")
export class DocumentTypeQueryController {
  #logger = new Logger(DocumentTypeQueryController.name);

  constructor(private readonly service: DocumentTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all documenttype with optional pagination" })
  @ApiResponse({ status: 200, type: DocumentTypesResponse })
  @ApiQuery({ name: "options", required: false, type: DocumentTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<DocumentType>    
  ): Promise<DocumentTypesResponse<DocumentType>> {
    try {
     
      const documenttypes = await this.service.findAll(options);
      logger.info("Retrieving all documenttype");
  ...
  @ApiOperation({ summary: "Find documenttype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter documenttype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: DocumentTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "DocumentType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find documenttypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<DocumentType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all documenttypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count documenttypes with conditions" })
  @ApiResponse({ status: 200, type: DocumentTypesResponse<DocumentType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one documenttype with conditions" })
  ...
  @ApiOperation({ summary: "Find one documenttype or return error" })
  @ApiResponse({ status: 200, type: DocumentTypeResponse<DocumentType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get documenttype by ID" })
  @ApiResponse({ status: 200, type: DocumentTypeResponse<DocumentType> })
  @ApiResponse({ status: 404, description: "DocumentType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the documenttype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<DocumentTypeResponse<DocumentType>> {
    try {
      const documenttype = await this.service.findOne({ where: { id } });
      if (!documenttype) {
        throw new NotFoundException(

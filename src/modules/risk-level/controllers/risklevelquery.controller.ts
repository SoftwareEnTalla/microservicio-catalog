...
import { RiskLevelDto } from "../dtos/all-dto";
...
@ApiTags("RiskLevel Query")
@UseGuards(RiskLevelAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("risklevels/query")
export class RiskLevelQueryController {
  #logger = new Logger(RiskLevelQueryController.name);

  constructor(private readonly service: RiskLevelQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all risklevel with optional pagination" })
  @ApiResponse({ status: 200, type: RiskLevelsResponse })
  @ApiQuery({ name: "options", required: false, type: RiskLevelDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<RiskLevel>    
  ): Promise<RiskLevelsResponse<RiskLevel>> {
    try {
     
      const risklevels = await this.service.findAll(options);
      logger.info("Retrieving all risklevel");
      return risklevels;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find risklevel by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter risklevel', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: RiskLevelsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "RiskLevel no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find risklevels with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<RiskLevel>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all risklevels" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count risklevels with conditions" })
  @ApiResponse({ status: 200, type: RiskLevelsResponse<RiskLevel> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades RiskLevels no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one risklevel with conditions" })
  ...
  @ApiOperation({ summary: "Find one risklevel or return error" })
  @ApiResponse({ status: 200, type: RiskLevelResponse<RiskLevel> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad RiskLevel no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get risklevel by ID" })
  @ApiResponse({ status: 200, type: RiskLevelResponse<RiskLevel> })
  @ApiResponse({ status: 404, description: "RiskLevel not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the risklevel to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<RiskLevelResponse<RiskLevel>> {
    try {
      const risklevel = await this.service.findOne({ where: { id } });
      if (!risklevel) {
        throw new NotFoundException(
          "RiskLevel no encontrado para el id solicitado"
        );
      }
      return risklevel;
    } catch (error) {

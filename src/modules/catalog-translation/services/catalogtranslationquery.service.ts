...
@Injectable()
export class CatalogTranslationQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(CatalogTranslationQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: CatalogTranslationQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${CatalogTranslation.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const catalogtranslations = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de catalogtranslations obtenido con éxito",
        data: catalogtranslations,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          catalogtranslations.length
        ),
        count: catalogtranslations.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<CatalogTranslationResponse<CatalogTranslation>> {
    try {
      const catalogtranslation = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "CatalogTranslations no encontrados para la propiedad y valor especificado"
        );
      // Devolver catalogtranslation
      return {
        ok: true,
        message: "CatalogTranslations obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<CatalogTranslationResponse<CatalogTranslation>> {
    try {

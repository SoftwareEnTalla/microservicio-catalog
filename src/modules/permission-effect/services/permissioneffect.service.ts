...
@Injectable()
export class PermissionEffectService {
  constructor(private readonly repository: PermissionEffectRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<PermissionEffect[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<PermissionEffect[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<PermissionEffect[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<PermissionEffect | null> {
  ...
  async update(id: string, partialEntity: Partial<PermissionEffect>): Promise<PermissionEffect | null> {

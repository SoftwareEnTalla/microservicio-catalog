...
import {ActiveStatusRepository} from './activestatus.repository'
...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class ActiveStatusQueryRepository {

    //Constructor del repositorio de datos: ActiveStatusQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination ActiveStatus on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[ActiveStatus[], number]> {
      logger.info('Ready to findAndCount ActiveStatus on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of ActiveStatus on repository:',result);
      return result;
    }

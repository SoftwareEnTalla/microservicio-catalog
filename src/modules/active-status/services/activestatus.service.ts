...
export class ActiveStatusService {
  constructor(private readonly repository: ActiveStatusRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<ActiveStatus[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<ActiveStatus[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<ActiveStatus[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<ActiveStatus | null> {
  ...
  async update(id: string, partialEntity: Partial<ActiveStatus>): Promise<ActiveStatus | null> {

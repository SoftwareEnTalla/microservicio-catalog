...
@ApiTags("ActiveStatus Query")
@UseGuards(ActiveStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("activestatuss/query")
export class ActiveStatusQueryController {
  #logger = new Logger(ActiveStatusQueryController.name);

  constructor(private readonly service: ActiveStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all activestatus with optional pagination" })
  @ApiResponse({ status: 200, type: ActiveStatussResponse })
  @ApiQuery({ name: "options", required: false, type: ActiveStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ActiveStatus>    
  ): Promise<ActiveStatussResponse<ActiveStatus>> {
    try {
     
      const activestatuss = await this.service.findAll(options);
      logger.info("Retrieving all activestatus");
  ...
  @ApiOperation({ summary: "Find activestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter activestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ActiveStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ActiveStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find activestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ActiveStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all activestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count activestatuss with conditions" })
  @ApiResponse({ status: 200, type: ActiveStatussResponse<ActiveStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one activestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one activestatus or return error" })
  @ApiResponse({ status: 200, type: ActiveStatusResponse<ActiveStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get activestatus by ID" })
  @ApiResponse({ status: 200, type: ActiveStatusResponse<ActiveStatus> })
  @ApiResponse({ status: 404, description: "ActiveStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the activestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ActiveStatusResponse<ActiveStatus>> {
    try {
      const activestatus = await this.service.findOne({ where: { id } });
      if (!activestatus) {
        throw new NotFoundException(

...
export class CatalogCommandService implements OnModuleInit {
  ...
  private async publishDslDomainEvents(events: BaseEvent[]): Promise<void> {
    for (const event of events) {
      ...
      if (process.env.EVENT_STORE_ENABLED === "true") {
        await this.eventStore.appendEvent('catalog-' + event.aggregateId, event);
      }
    }
  }
  ...
  async create(
    ...
    try {
      ...
      const candidate = Catalog.fromDto(createCatalogDtoInput);
      await this.applyDslServiceRules("create", createCatalogDtoInput as Record<string, any>, candidate, null, false);
      const entity = await this.repository.create(candidate);
      await this.applyDslServiceRules("create", createCatalogDtoInput as Record<string, any>, entity, null, true);
  ...
  async update(
    ...
    try {
      ...
      const candidate = Object.assign(new Catalog(), currentEntity ?? {}, partialEntity);

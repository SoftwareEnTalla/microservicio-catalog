...
export class CatalogCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Catalog): Promise<Catalog> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new CatalogCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Catalog[]): Promise<Catalog[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new CatalogCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new CatalogUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new CatalogUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new CatalogDeletedEvent(id, {
        instance: entity,
        metadata: {

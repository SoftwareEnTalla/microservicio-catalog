...
import { CatalogDto } from "../dtos/all-dto";

import { logger } from '@core/logs/logger';

@ApiTags("Catalog Query")
@Controller("catalogs/query")
export class CatalogQueryController {
  #logger = new Logger(CatalogQueryController.name);

  constructor(private readonly service: CatalogQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all catalog with optional pagination" })
  @ApiResponse({ status: 200, type: CatalogsResponse })
  @ApiQuery({ name: "options", required: false, type: CatalogDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Catalog>    
  ): Promise<CatalogsResponse<Catalog>> {
    try {
     
      const catalogs = await this.service.findAll(options);
      logger.info("Retrieving all catalog");
      return catalogs;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the catalog to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CatalogResponse<Catalog>> {
    try {
      const catalog = await this.service.findOne({ where: { id } });
      if (!catalog) {
        throw new NotFoundException(
          "Catalog no encontrado para el id solicitado"
        );
      }
      return catalog;
    } catch (error) {
  ...
  @ApiOperation({ summary: "Find catalog by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter catalog', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CatalogsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Catalog no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find catalogs with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Catalog>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all catalogs" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count catalogs with conditions" })
  @ApiResponse({ status: 200, type: CatalogsResponse<Catalog> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Catalogs no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one catalog with conditions" })
  @ApiResponse({ status: 200, type: CatalogResponse<Catalog> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one catalog or return error" })
  @ApiResponse({ status: 200, type: CatalogResponse<Catalog> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Catalog no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);

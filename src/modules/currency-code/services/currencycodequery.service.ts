...
@Injectable()
export class CurrencyCodeQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(CurrencyCodeQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: CurrencyCodeQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${CurrencyCode.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const currencycodes = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de currencycodes obtenido con éxito",
        data: currencycodes,
  ...
  async findById(id: string): Promise<CurrencyCodeResponse<CurrencyCode>> {
    try {
      const currencycode = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "CurrencyCodes no encontrados para la propiedad y valor especificado"
        );
      // Devolver currencycode
      return {
        ok: true,
        message: "CurrencyCodes obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<CurrencyCodeResponse<CurrencyCode>> {
    try {

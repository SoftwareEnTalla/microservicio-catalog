...
import { CatalogItemQueryRepository } from './catalogitemquery.repository';
import { generateCacheKey } from 'src/utils/functions';
import { Cacheable } from '../decorators/cache.decorator';
import {CatalogItemRepository} from './catalogitem.repository';
...
export class CatalogItemCommandRepository implements IEventHandler<BaseEvent>{
  ...
  //Constructor del repositorio de datos: CatalogItemCommandRepository
  constructor(
    @InjectRepository(CatalogItem)
    private readonly repository: Repository<CatalogItem>,
    private readonly catalogitemRepository: CatalogItemQueryRepository,
  ...
  async handle(event: any) {
    ...
    logger.info('Ready to handle CatalogItem event on repository:', event);
    switch (event.constructor.name) {
      case 'CatalogItemCreatedEvent':
        return await this.onCatalogItemCreated(event);
      case 'CatalogItemUpdatedEvent':
  ...
  @Cacheable({
    key: (args) => generateCacheKey<CatalogItem>('createCatalogItem', args[0], args[1]),
    ttl: 60,
  })
  private async onCatalogItemCreated(event: CatalogItemCreatedEvent) {
    logger.info('Ready to handle onCatalogItemCreated event on repository:', event);
    const entity = new CatalogItem();
    entity.id = event.aggregateId;
    Object.assign(entity, event.payload.instance);
    // Asegurar que el tipo discriminador esté establecido
    if (!entity.type) {
      entity.type = 'catalogitem';
    }
  ...
  @Cacheable({
    key: (args) => generateCacheKey<CatalogItem>('updateCatalogItem', args[0], args[1]),
    ttl: 60,
  })
  private async onCatalogItemUpdated(event: CatalogItemUpdatedEvent) {
    logger.info('Ready to handle onCatalogItemUpdated event on repository:', event);
  ...
  @Cacheable({
    key: (args) => generateCacheKey<CatalogItem>('deleteCatalogItem', args[0], args[1]),
    ttl: 60,
  })
  private async onCatalogItemDeleted(event: CatalogItemDeletedEvent) {
    logger.info('Ready to handle onCatalogItemDeleted event on repository:', event);
  ...
  async create(entity: CatalogItem): Promise<CatalogItem> {
    ...
    if (this.shouldPublishEvent()) {
      const event = new CatalogItemCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: CatalogItem[]): Promise<CatalogItem[]> {
    ...
    if (this.shouldPublishEvent()) {
      const events = result.map((el) => new CatalogItemCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    let instance=await this.catalogitemRepository.findById(id);
    logger.info('Updated instance of CatalogItem with id: ${id} was finded on repository:', instance);
    
    if(instance && this.shouldPublishEvent()) {
      logger.info('Ready to publish or fire event CatalogItemUpdatedEvent on repository:', instance);
      const event = new CatalogItemUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const updateEvent = new CatalogItemUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     const entity = await this.catalogitemRepository.findOne({ id });
     if(!entity){
     ...
     if (this.shouldPublishEvent()) {
       logger.info('Ready to publish/fire CatalogItemDeletedEvent on repository:', result);
       const event = new CatalogItemDeletedEvent(id, {
        instance: entity,
        metadata: {
  ...
  async bulkDelete(ids: string[]): Promise<DeleteResult> {
    ...
    if (this.shouldPublishEvent()) {
      ...
      const deleteEvents = await Promise.all(ids.map(async (id) => {
          ...
          return new CatalogItemDeletedEvent(id, {
            instance: entity,
            metadata: {
              initiatedBy: entity.createdBy || 'system',
              correlationId: entity.id,

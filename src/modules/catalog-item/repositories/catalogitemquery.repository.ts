...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class CatalogItemQueryRepository {

    //Constructor del repositorio de datos: CatalogItemQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination CatalogItem on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[CatalogItem[], number]> {
      logger.info('Ready to findAndCount CatalogItem on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of CatalogItem on repository:',result);
      return result;
    }
    ...
    client: LoggerClient.getInstance()
      ...
      .get(CatalogItemRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<CatalogItem | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) CatalogItem:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<CatalogItem> = options as FindOptionsWhere<CatalogItem>;

...
export class CatalogItemService {
  ...
  // Métodos delegados
  async findAll(options?: any): Promise<CatalogItem[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<CatalogItem[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<CatalogItem[]> {
  ...
  async update(id: string, partialEntity: Partial<CatalogItem>): Promise<CatalogItem | null> {

...
@ApiTags("CatalogItem Query")
@UseGuards(CatalogItemAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("catalogitems/query")
export class CatalogItemQueryController {
  #logger = new Logger(CatalogItemQueryController.name);

  constructor(private readonly service: CatalogItemQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all catalogitem with optional pagination" })
  @ApiResponse({ status: 200, type: CatalogItemsResponse })
  @ApiQuery({ name: "options", required: false, type: CatalogItemDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CatalogItem>    
  ): Promise<CatalogItemsResponse<CatalogItem>> {
    try {
     
      const catalogitems = await this.service.findAll(options);
      logger.info("Retrieving all catalogitem");
      return catalogitems;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find catalogitem by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter catalogitem', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CatalogItemsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CatalogItem no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find catalogitems with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CatalogItem>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all catalogitems" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count catalogitems with conditions" })
  @ApiResponse({ status: 200, type: CatalogItemsResponse<CatalogItem> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades CatalogItems no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one catalogitem with conditions" })
  ...
  @ApiOperation({ summary: "Find one catalogitem or return error" })
  @ApiResponse({ status: 200, type: CatalogItemResponse<CatalogItem> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad CatalogItem no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get catalogitem by ID" })
  @ApiResponse({ status: 200, type: CatalogItemResponse<CatalogItem> })
  @ApiResponse({ status: 404, description: "CatalogItem not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the catalogitem to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CatalogItemResponse<CatalogItem>> {
    try {
      const catalogitem = await this.service.findOne({ where: { id } });
      if (!catalogitem) {
        throw new NotFoundException(
          "CatalogItem no encontrado para el id solicitado"
        );
      }
      return catalogitem;
    } catch (error) {

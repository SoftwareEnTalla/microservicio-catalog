...
import { LifecycleStatusDto } from "../dtos/all-dto";
...
@ApiTags("LifecycleStatus Query")
@UseGuards(LifecycleStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("lifecyclestatuss/query")
export class LifecycleStatusQueryController {
  #logger = new Logger(LifecycleStatusQueryController.name);

  constructor(private readonly service: LifecycleStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all lifecyclestatus with optional pagination" })
  @ApiResponse({ status: 200, type: LifecycleStatussResponse })
  @ApiQuery({ name: "options", required: false, type: LifecycleStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<LifecycleStatus>    
  ): Promise<LifecycleStatussResponse<LifecycleStatus>> {
    try {
     
      const lifecyclestatuss = await this.service.findAll(options);
      logger.info("Retrieving all lifecyclestatus");
  ...
  @ApiOperation({ summary: "Find lifecyclestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter lifecyclestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LifecycleStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "LifecycleStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find lifecyclestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<LifecycleStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all lifecyclestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count lifecyclestatuss with conditions" })
  @ApiResponse({ status: 200, type: LifecycleStatussResponse<LifecycleStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades LifecycleStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one lifecyclestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one lifecyclestatus or return error" })
  @ApiResponse({ status: 200, type: LifecycleStatusResponse<LifecycleStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad LifecycleStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get lifecyclestatus by ID" })
  @ApiResponse({ status: 200, type: LifecycleStatusResponse<LifecycleStatus> })
  @ApiResponse({ status: 404, description: "LifecycleStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the lifecyclestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LifecycleStatusResponse<LifecycleStatus>> {
    try {
      const lifecyclestatus = await this.service.findOne({ where: { id } });
      if (!lifecyclestatus) {
        throw new NotFoundException(
          "LifecycleStatus no encontrado para el id solicitado"
        );
      }
